...
GROQ_API_KEY = "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"
    ...
    if not st.session_state.get('agents_initialized', False):
        try:
            st.session_state.web_agent = Agent(
                ...
                model=Groq(api_key=GROQ_API_KEY, model="llama3.3-70b-versatile"),
                tools=[GoogleSearch(fixed_max_results=10), DuckDuckGo(fixed_max_results=10)]
            )
...
def main():
    ...
    stock_input = st.text_input("Enter Company Name or Ticker", help="e.g., APPLE, TCS, or any stock ticker")
    ...
    if st.button("Analyze", use_container_width=True):
        ...
        symbol = COMMON_STOCKS.get(user_input.upper(), user_input)
